...
def parsing_arguments():
    ...
    parser.add_argument("-x","--runningdir", type=str, required=False, default="./", help="Running directory (default: ./)")
    parser.add_argument("-n","--clones", type=str, required=False, default="2,8", help="Either an estimated number of clones or an interval where the\nnumber of clones should be looked for given in the form LOWER,UPPER where LOWER and UPPER are two integer defining the interval (default: 2,8)")
    parser.add_argument("-f","--noampdel", action='store_true', default=False, required=False, help="Remove amp-del assumption where each mutated allele of every segment can be either amplified or deleted in all tumor clones w.r.t. base (2 for diploid and 4 for tetraploid) (default: use assumption)")    
    parser.add_argument("-c","--clonal", type=str, required=False, default=None, help="Clonal clusters to fix for tetraploid (default: automatically inferred)")    
    parser.add_argument("-d","--cnstates", type=int, required=False, default=None, help="Maximum number of distinct copy-number states for each segment (default: None, no limit)")
    parser.add_argument("-eD","--diploidcmax", type=int, required=False, default=5, help="Maximum copy-number value overall segments (default: 5, 0 means inferred from scaled fractional copy numbers)")
    parser.add_argument("-eT","--tetraploidcmax", type=int, required=False, default=8, help="Maximum copy-number value overall segments (default: 8, 0 means inferred from scaled fractional copy numbers)")
    parser.add_argument("-ts","--minsize", type=float, required=False, default=0.02, help="The minimum proportion of covered genome for potential clonal clusters (default: 0.02)")
    parser.add_argument("-tc","--minchrs", type=int, required=False, default=1, help="The minimum number of covered chromosomes for potential clonal clusters (default: 4)")
    ...
    parser.add_argument("-g", "--ghostprop", type=float, required=False, default=0.3, help="Increasing proportion used to compute the value of the first ghost point added in the solution selection (default: 0.2)")
    parser.add_argument("-tR","--toleranceRDR", type=float, required=False, default=0.04, help="RDR tolerance used for finding the clonal copy numbers (default: 0.04)")
    parser.add_argument("-tB","--toleranceBAF", type=float, required=False, default=0.03, help="BAF tolerance used for finding the clonal copy numbers (default: 0.03)")
    parser.add_argument("-p","--seeds", type=int, required=False, default=100, help="Number of seeds for coordinate-descent method (default: 100)")
    ...
    parser.add_argument("-u","--minprop", type=float, required=False, default=None, help="Minimum clone proporion in each sample (default: 0.01)")
    parser.add_argument("--maxiterations", type=int, required=False, default=None, help="Maximum number of iterations composed of C-step/U-step for each seed (default: 10)")    
    parser.add_argument("--mode", type=int, required=False, default=2, help="Solving mode among: Coordinate Descent + exact ILP (0), exact ILP only (1), and Coordinate-descent only (2) (default: 2)")
...
def main():
    ...
    if not args['diploid'] and not args['tetraploid']:
    	...
    	if len(clonal) > 0:
            ...
            sys.stderr.write(warning("# No potential clonal patterns found, the input is likely to be diploid (also try to increase values of tB and tR for accounting more noisy data)\n"))
...
def select(diploid, tetraploid, v, rundir, g, limit):
    ...
    if tchosen[0] < dchosen[0]:
        ...
        if tchosen[0] == tetraploid[0][0] and False not in set(tscores[tet[0]] <= 0.05 for i, tet in enumerate(tetraploid) if i > 0):
            sys.stderr.write(warn("### Tetraploid has been chosen because OBJ did not vary significantly, this means that either there is only one tetraploid tumor clone or the heuristic identified wrong clonal segments. Run again the program by specifying different potential clonal segments to double check o try to use the parameters tR and tB!"))
    else:
