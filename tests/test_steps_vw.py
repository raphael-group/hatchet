...
def test_count_reads(_, bams, output_folder):
    ...
    truth1 = np.loadtxt(
        os.path.join(this_dir, "data", "vw", "rdr", "chr22.thresholds.gz")
    )
    truth2 = np.loadtxt(os.path.join(this_dir, "data", "vw", "rdr", "chr22.total.gz"))
    truth3 = [
        l for l in open(os.path.join(this_dir, "data", "vw", "rdr", "samples.txt"), "r")
    ]
...
def test_combine_counts(_, output_folder):
    ...
    df2 = pd.read_csv(
        os.path.join(this_dir, "data", "vw", "bb", "bulk_nophase.bb"), sep="\t"
    )
...
def test_cluster_bins(output_folder):
    ...
    seg2 = pd.read_csv(
        os.path.join(this_dir, "data", "vw", "bbc", "bulk.seg"), sep="\t"
    )
    bbc2 = pd.read_csv(
        os.path.join(this_dir, "data", "vw", "bbc", "bulk.bbc"), sep="\t"
    )
...
def test_cluster_bins_singleton(output_folder):
    ...
    seg2 = pd.read_csv(
        os.path.join(this_dir, "data", "vw", "bbc", "bulk_onecl.seg"), sep="\t"
    )
    bbc2 = pd.read_csv(
        os.path.join(this_dir, "data", "vw", "bbc", "bulk_onecl.bbc"), sep="\t"
    )
    assert_frame_equal(seg1, seg2)
    assert_frame_equal(bbc1, bbc2)
