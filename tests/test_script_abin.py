...
@pytest.fixture(scope='module')
def output_folder():
    out = os.path.join(this_dir, 'out_abin')
    shutil.rmtree(out, ignore_errors=True)
    for sub_folder in ('rdr', 'snps', 'baf', 'bb', 'bbc', 'plots', 'results', 'evaluation', 'analysis'):
...
def test_script(_, bams, output_folder):
    ...
    df2 = pd.read_csv(os.path.join(this_dir, 'data', 'bulk_abin.seg'), sep='\t')
    ...
    if solver_available():
        ...
        df2 = pd.read_csv(os.path.join(this_dir, 'data', 'best_abin.bbc.ucn'), sep='\t')
        assert_frame_equal(df1, df2)
