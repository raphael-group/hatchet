...
def parse_args():
    ...
    parser.add_argument("INPUT", type=str, help='A white-space-separated list between apices where each element is a segmented CNV file produced by GATK4 CNV pipeline. The file format is describe in the HATCHet\'s repository.')
    parser.add_argument("--samples", type=str, required=False, default=None, help="A white-space-separated list containing the name of sample in the same order as given (default: filenames are used).")
    parser.add_argument("-b","--binsize", required=False, default='50kb', type=str, help="Size of the bins, specified as a full number or using the notations either \"kb\" or \"Mb\" (default: 50kb).")
...
def binning(segs, size, drdr, dbaf):
    ...
    gen = (lambda d : {'RDR' : norm(d['RDR'], drdr), 'BAF' : BAF(norm(d['BAF'], dbaf))})
