...
def compute_loglik_per_sample(
    X_rdrs, X_alphas, X_betas, X_totals, rdr_means, rdr_vars, baf_means, baf_taus
):
    """Per-bin per-cluster per-sample log-likelihoods (sample axis retained).

    Same inputs as compute_loglik but does NOT sum over the sample axis M,
    allowing downstream code to inspect per-sample fit quality.
    ...
    bb_alpha = baf_taus[None, None, :] * baf_means[None, :, :]
    bb_beta = baf_taus[None, None, :] * (1 - baf_means[None, :, :])
    bb_delta = betaln(bb_alpha, bb_beta)
    lnB_h0 = betaln(X_alphas[:, None, :] + bb_alpha, X_betas[:, None, :] + bb_beta)
    lnB_h1 = betaln(X_betas[:, None, :] + bb_alpha, X_alphas[:, None, :] + bb_beta)
    ll_baf_h0 = log_binom_const[:, None, :] + lnB_h0 - bb_delta  # (N, K, M)
    ll_baf_h1 = log_binom_const[:, None, :] + lnB_h1 - bb_delta
...
def compute_loglik_single_cluster_batch(
    ...
    Computes the same emission model as compute_loglik_per_sample but for
    C candidate clusters at once, reusing a precomputed log_binom_const to
    avoid redundant gammaln evaluations across calls.
...
def compute_loglik_decomposed(
    X_rdrs, X_alphas, X_betas, X_totals, rdr_means, rdr_vars, baf_means, baf_taus
):
    """Like compute_loglik but returns BAF and RDR components separately.
    ...
    lnB_h0 = betaln(X_alphas[:, None, :] + bb_alpha, X_betas[:, None, :] + bb_beta)
    lnB_h1 = betaln(X_betas[:, None, :] + bb_alpha, X_alphas[:, None, :] + bb_beta)
    ll_bafs_h0 = np.sum(log_binom_const[:, None, :] + lnB_h0 - bb_delta, axis=2)
    ll_bafs_h1 = np.sum(log_binom_const[:, None, :] + lnB_h1 - bb_delta, axis=2)
    ...
    return ll_rdrs, ll_bafs_h0, ll_bafs_h1
