...
from hatchet.utils.ArgParsing import parse_cluster_bins_gmm_args
import hatchet.utils.Supporting as sp


def main(args=None):
    sp.log(msg="# Parsing and checking input arguments\n", level="STEP")
    args = parse_cluster_bins_gmm_args(args)
    sp.logArgs(args, 80)
    ...
    clusters = {
        cluster: set(
            key for key in combo if clusterAssignments[bintoidx[key]] == cluster
        )
...
def refineClustering(combo, assign, assignidx, samples, rdtol, baftol):
    ...
    getbaf = lambda c, p: float(
        sum(e[6] for b in combo for e in combo[b] if assignment[b] == c and e[0] == p)
    )
    baf = {c: {p: getbaf(c, p) / size[c] for p in samples} for c in clusters}
    getrdr = lambda c, p: float(
        sum(e[1] for b in combo for e in combo[b] if assignment[b] == c and e[0] == p)
    )
...
def generateClouds(points, density, seed, sdeven=0.02, sdodd=0.02):
    ...
    for point in points:
        ...
        for d in range(density):
            ...
            newpoint = [
                normal(point[i], sdeven) if i % 2 == 0 else normal(point[i], sdodd)
...
def minSegmentBins(sbb, nbins, rd, nsnps, cov, clusters, samples):
    alpha = {
        cluster: {
            sample: sum(
                min(sbb[bi][sample][3], sbb[bi][sample][4]) for bi in clusters[cluster]
            )
            for sample in samples
        }
        for cluster in clusters
    ...
    beta = {
        cluster: {
            sample: sum(
                max(sbb[bi][sample][3], sbb[bi][sample][4]) for bi in clusters[cluster]
            )
            for sample in samples
        }
        for cluster in clusters
...
def scaleBAF(segments, samples, diploidbaf):
    ...
    for key in segments:
        if sum(
            (0.5 - segments[key][sample][-1]) <= diploidbaf for sample in samples
        ) == len(samples):
    ...
    scaledBAF = {
        segment: {
            sample: scale(segments[segment][sample][-1], scalings[sample], diploidbaf)
            for sample in samples
        }
    ...
    return {
        segment: {
            sample: regulate(segments[segment][sample], scaledBAF[segment][sample])
            for sample in samples
        }
        for segment in segments
    }
