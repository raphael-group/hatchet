...
def multisample_em(alts, refs, start, tol = 10e-6):
    assert refs.shape == alts.shape, "Alternate and reference count arrays have different shapes"
    ...
    log_likelihood = bmm_ll_multi(alts + refs, alts, theta, 0.5)
    return theta, phases, log_likelihood

def bmm_ll_multi(d_mat,x_mat,p_list,alpha):
    n,m=d_mat.shape
    p_mat=np.tile(p_list,(m,1)).T    
    
    return np.sum( np.log( alpha * binom.pmf(x_mat, d_mat,p_mat) + (1-alpha) * binom.pmf(d_mat-x_mat, d_mat, p_mat) ))
