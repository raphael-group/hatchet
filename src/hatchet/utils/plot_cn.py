...
def parsing_arguments(args=None):
    ...
    if len(args.INPUT.split()) != len(set(patientnames.values())):
        raise ValueError(
            error("Multiple patients have the same name but they should unique!")
        )
    ...
    if not 0.0 <= args.minu <= 1.0:
        raise ValueError(
            error("The minimum proportion for subclonal CNAs must be in [0, 1]!")
        )
...
def main(args=None):
    ...
    sys.stderr.write(
        log("# Compute purity and tumor ploidy of each sample from each patient\n")
    )
...
def pp(tumor, clones, props, args):
    ...
    for patient in tumor:
        ...
        for sample in props[patient]:
            ...
            length = sum(
                float(s[1] - s[0]) for c in tumor[patient] for s in tumor[patient][c]
            )
            ploidy = (
                sum(
...
def profiles(tumor, clones, props, args, out):
    ...
    if len(tclones) % 2 == 0:
        available = [-shift / 2] + [
            -shift / 2 - shift * (i + 1) for i in range(int(len(tclones) / 2) - 1)
        ]
        available += [shift / 2] + [
            shift / 2 + shift * (i + 1) for i in range(int(len(tclones) / 2) - 1)
        ]
    ...
    with PdfPages(os.path.join(args["rundir"], out)) as pdf:
        for sample in props:
            ...
            merge = {
                c: {
                    s: {
                        sum(cn): sumu(proj[c][s], cn) for cn in set(proj[c][s].values())
                    }
                    for s in proj[c]
                }
                for c in proj
            ...
            palA = iter(
                sns.color_palette("YlOrRd_r", len([cn for cn in cns if cn > base]))
            )
            for x in [cn for cn in cns if cn > base]:
                style[x] = next(palA)
            # palD = iter(sns.color_palette("Blues", len([cn for cn in cns if cn < base])))
            palD = iter(
                sns.color_palette("YlGnBu", len([cn for cn in cns if cn < base]))
            )
            for x in [cn for cn in cns if cn < base]:
                style[x] = next(palD)
...
def allelicproportions(tumor, base, clones, props, args, out):
    with PdfPages(os.path.join(args["rundir"], out)) as pdf:
        for sample in props:
            ...
            mergeA = {
                c: {
                    s: {cn[0]: sumu(proj[c][s], cn) for cn in set(proj[c][s].values())}
                    for s in proj[c]
                }
                for c in proj
            }
            ...
            mergeB = {
                c: {
                    s: {cn[1]: sumu(proj[c][s], cn) for cn in set(proj[c][s].values())}
                    for s in proj[c]
                }
                for c in proj
            }
            ...
            # palA = iter(sns.color_palette("Reds", len([cn for cn in cns if cn > base])))
            palA = iter(
                sns.color_palette("YlOrRd", len([cn for cn in cns if cn > base]))
            )
            for x in [cn for cn in cns if cn > base]:
                style[x] = next(palA)
            # palD = iter(sns.color_palette("Blues", len([cn for cn in cns if cn < base])))
...
def gridprofiles(tumor, base, clones, props, args, out):
    ...
    for c in sorted([i for i in clones]):
        for x, s in enumerate(pos):
            ...
            row_colors[c] = {
                sam: pal_sample[min(9, int(round(props[sam][c] * 10)))] for sam in props
            }
    if len(am) == 0:
        am.add(base)
    if len(de) == 0:
...
def gridmixtures(tumor, base, clones, props, args, out):
    ...
    for p in props:
        ...
        mergep = {
            c: {
                s: {cn: sumu(redp[c][s], cn) for cn in set(redp[c][s].values())}
                for s in redp[c]
            }
            for c in redp
        }
        ...
        for x, s in enumerate(pos):
            ...
            col_colors[x] = chr_colors[s[0]]
            row_colors[p] = {
                i: pal_clone[min(9, int(round(props[p][i] * 10)))] for i in clones
            }
...
def reduction(proj, base):
    ...
    reduction = {
        c: {
            s: {i: classify(sum(proj[c][s][i]), i) for i in proj[c][s]} for s in proj[c]
        }
        for c in proj
    }
    assert False not in set(
...
def segmenting(tumor, clones, props):
    ...
    return {
        pat: {c: {b: tumor[pat][c][maps[pat][c][b]] for b in taken[c]} for c in taken}
        for pat in tumor
    }
...
def similaritysample(u, v):
    ...
    a = float(
        sum(
            (bothamp(u[i], v[i]) or bothdel(u[i], v[i])) and u[i] != 0 and v[i] != 0
            ...
            for line in f:
                if len(clones[patient]) == 0:
                    ...
                    if "normal" not in clones[patient]:
                        raise ValueError(
                            error(
                                "normal is not present as a clone in {}".format(patient)
                            )
                        )
                else:
