...
def solve(clonal, bbc_file, seg_file, n, solver='gurobi', solve_mode='cd', d=-1, cn_max=-1, mu=0.01, diploid_threshold=0.1,
          ampdel=True, n_seed=400, n_worker=8, random_seed=None, max_iters=None, timelimit=None, binwise = False):
    ...
    sp.log(msg = "Computed scaling factor gamma = \n" + str(gamma) + '\n', level = 'INFO')
    rdr = rdr * gamma
    f_b = rdr * baf
    f_a = rdr - f_b
    ...
    if not binwise:
        ...
        bins_rdr = {k:my_df.pivot(index=['#CHR', 'START'], columns='SAMPLE', values='RD') for k, my_df in bbc.groupby('CLUSTER')}
        bins_baf = {k:my_df.pivot(index=['#CHR', 'START'], columns='SAMPLE', values='BAF') for k, my_df in bbc.groupby('CLUSTER')}
        bins_length = {k:((my_df.pivot(index=['#CHR', 'START'], columns='SAMPLE', values='END') - 
                        my_df.pivot(index=['#CHR', 'START'], columns='SAMPLE', values='START')).values[:, 0] * 100) / binned_length for k, my_df in bbc.groupby('CLUSTER')}
        ...
        if solve_mode == 'ilp':
            ilp = ILPSubsetSplit(n, cn_max, d=d, mu=mu, ampdel=ampdel, copy_numbers=copy_numbers, f_a=f_a, f_b=f_b, binsA = binsA, binsB = binsB, lengths = bins_length)
            ilp.create_model(pprint=True)
            return ilp.run(solver_type=solver, timelimit=timelimit)
        elif solve_mode == 'cd':
            cd = CoordinateDescentSplit(f_a=f_a, f_b=f_b, n=n, mu=mu, d=d, cn_max=cn_max, ampdel=ampdel,
                                cn=copy_numbers, binsA = binsA, binsB = binsB, lengths = bins_length)
            return cd.run(solver_type=solver, max_iters=max_iters, n_seed=n_seed, j=n_worker, random_seed=random_seed, timelimit=timelimit)
        else:
            cd = CoordinateDescentSplit(f_a=f_a, f_b=f_b, n=n, mu=mu, d=d, cn_max=cn_max, ampdel=ampdel,
                                cn=copy_numbers, binsA = binsA, binsB = binsB, lengths = bins_length)
            ...
            ilp = ILPSubsetSplit(n, cn_max, d=d, mu=mu, ampdel=ampdel, copy_numbers=copy_numbers, f_a=f_a, f_b=f_b, cn=copy_numbers, binsA = binsA, binsB = binsB, lengths = bins_length)
            ilp.create_model()
            ilp.hot_start(cA, cB)
            return ilp.run(solver_type=solver, timelimit=timelimit)
