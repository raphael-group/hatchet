...
class ILPSubset:
    ...
    def create_model(self):
        ...
        for _m in range(m):
            ...
            for _k in range(k):
                yA[(_m, _k)] = model.addVar(lb=0, ub=GRB.INFINITY, obj=0, vtype=GRB.CONTINUOUS,
                                            name=f'yA_{_m + 1}_{_k + 1}')
                yB[(_m, _k)] = model.addVar(lb=0, ub=GRB.INFINITY, obj=0, vtype=GRB.CONTINUOUS,
                                            name=f'yB_{_m + 1}_{_k + 1}')
                fA[(_m, _k)] = model.addVar(lb=0, ub=ub, obj=0, vtype=GRB.CONTINUOUS, name=f'fA_{_m + 1}_{_k + 1}')
                fB[(_m, _k)] = model.addVar(lb=0, ub=ub, obj=0, vtype=GRB.CONTINUOUS, name=f'fB_{_m + 1}_{_k + 1}')
        ...
        if mode_t in ('FULL', 'CARCH'):
            for _m in range(m):
                ...
                for _n in range(n):
                    self.cA.iloc[_m][_n] = model.addVar(lb=0, ub=ub, obj=0, vtype=GRB.INTEGER, name=f'cA_{_m + 1}_{_n + 1}')
                    self.cB.iloc[_m][_n] = model.addVar(lb=0, ub=ub, obj=0, vtype=GRB.INTEGER, name=f'cB_{_m + 1}_{_n + 1}')

            if ampdel:
                for _m in range(m):
                    cluster_id = f_a.index[_m]
                    if cluster_id not in copy_numbers:
                        adA[_m] = model.addVar(lb=0, ub=1, obj=0, vtype=GRB.BINARY, name=f'adA_{_m + 1}')
                        adB[_m] = model.addVar(lb=0, ub=1, obj=0, vtype=GRB.BINARY, name=f'adB_{_m + 1}')
        ...
        if (mode_t == 'FULL') or (d > 0 and mode_t == 'CARCH'):
            for _b in range(_M):
                for _m in range(m):
                    for _n in range(n):
                        bitcA[(_b, _m, _n)] = model.addVar(lb=0, ub=1, obj=0, vtype=GRB.BINARY,
                                                           name=f'bitcA_{_b + 1}_{_m + 1}_{_n + 1}')
                        bitcB[(_b, _m, _n)] = model.addVar(lb=0, ub=1, obj=0, vtype=GRB.BINARY,
                                                           name=f'bitcB_{_b + 1}_{_m + 1}_{_n + 1}')

        if mode_t in ('FULL', 'UARCH'):
            for _n in range(n):
                for _k in range(k):
                    self.u.iloc[_n][_k] = model.addVar(lb=0, ub=1, obj=0, vtype=GRB.CONTINUOUS, name=f'u_{_n + 1}_{_k + 1}')
        ...
        if mode_t == 'FULL':
            for _b in range(_M):
                for _m in range(m):
                    for _n in range(n):
                        for _k in range(k):
                            vA[(_b, _m, _n, _k)] = model.addVar(lb=0, ub=1, obj=0, vtype=GRB.CONTINUOUS,
                                                                name=f'vA_{_b + 1}_{_m + 1}_{_n + 1}_{_k + 1}')
                            vB[(_b, _m, _n, _k)] = model.addVar(lb=0, ub=1, obj=0, vtype=GRB.CONTINUOUS,
        ...
        for _m in range(m):
            ...
            for _k in range(k):
                ...
                model.addConstr(f_a_values[_k] - _fA <= _yA)
                model.addConstr(_fA - f_a_values[_k] <= _yA)
                model.addConstr(f_b_values[_k] - _fB <= _yB)
                model.addConstr(_fB - f_b_values[_k] <= _yB)
        ...
        if mode_t == 'FULL':
                ...
                for _k in range(k):
                    ...
                    for _n in range(n):
                        for _b in range(_M):
                            ...
                            model.addConstr(vA[(_b, _m, _n, _k)] <= bitcA[(_b, _m, _n)])
                            model.addConstr(vA[(_b, _m, _n, _k)] <= self.u.iloc[_n][_k])
                            model.addConstr(vA[(_b, _m, _n, _k)] >= bitcA[(_b, _m, _n)] + self.u.iloc[_n][_k] - 1)

                    model.addConstr(fA[(_m, _k)] == sum_a)
                ...
                for _k in range(k):
                    ...
                    for _n in range(n):
                        for _b in range(_M):
                            ...
                            model.addConstr(vB[(_b, _m, _n, _k)] <= bitcB[(_b, _m, _n)])
                            model.addConstr(vB[(_b, _m, _n, _k)] <= self.u.iloc[_n][_k])
                            model.addConstr(vB[(_b, _m, _n, _k)] >= bitcB[(_b, _m, _n)] + self.u.iloc[_n][_k] - 1)

                    model.addConstr(fB[(_m, _k)] == sum_b)
            ...
            for _n in range(n):
                for _k in range(k):
                    ...
                    model.addConstr(_sum >= self.u.iloc[_n][_k])
        ...
        if (mode_t == 'FULL') or (d > 0 and mode_t == 'CARCH'):
            for _m in range(m):
                ...
                for _n in range(n):
                    ...
                    model.addConstr(self.cA.iloc[_m][_n] == sum_a)
                    model.addConstr(self.cB.iloc[_m][_n] == sum_b)
                    model.addConstr(self.cA.iloc[_m][_n] + self.cB.iloc[_m][_n] <= ub)
        ...
        if mode_t in ('FULL', 'CARCH'):
            for _m in range(m):
                model.addConstr(self.cA.iloc[_m][0] == 1)
                model.addConstr(self.cB.iloc[_m][0] == 1)
            ...
            if ampdel:
                for _m in range(m):
                    ...
                    if cluster_id not in copy_numbers:
                        for _n in range(1, n):
                            model.addConstr(self.cA.iloc[_m][_n] <= cn_max * adA[_m] + _base - _base * adA[_m])
                            model.addConstr(self.cA.iloc[_m][_n] >= _base * adA[_m])
                            model.addConstr(self.cB.iloc[_m][_n] <= cn_max * adB[_m] + _base - _base * adB[_m])
                            model.addConstr(self.cB.iloc[_m][_n] >= _base * adB[_m])
    ...
    def fix_given_cn(self, model):
        for _m in range(self.m):
            ...
            if cluster_id in self.copy_numbers:
                ...
                for _n in range(1, self.n):
                    model.addConstr(self.cA.iloc[_m][_n] == _cnA)
    ...
    def run(self, write_path=None):
        ...
        def _value(var): return getattr(var, 'X', var)
