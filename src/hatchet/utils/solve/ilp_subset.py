...
class ILPSubset:
    def __init__(self, n, cn_max, d, mu, ampdel, copy_numbers, f_a, f_b, w):
        ...
        self.model1 = None  # initialized on create_model()
    ...
    def create_model(self):
        ...
        for _m in range(m):
            ...
            for _k in range(k):
                yA[(_m, _k)] = model1.addVar(lb=0, ub=GRB.INFINITY, obj=0, vtype=GRB.CONTINUOUS,
                                            name=f'yA_{_m + 1}_{_k + 1}')
                yB[(_m, _k)] = model1.addVar(lb=0, ub=GRB.INFINITY, obj=0, vtype=GRB.CONTINUOUS,
                                            name=f'yB_{_m + 1}_{_k + 1}')
                fA[(_m, _k)] = model1.addVar(lb=0, ub=ub, obj=0, vtype=GRB.CONTINUOUS, name=f'fA_{_m + 1}_{_k + 1}')
                fB[(_m, _k)] = model1.addVar(lb=0, ub=ub, obj=0, vtype=GRB.CONTINUOUS, name=f'fB_{_m + 1}_{_k + 1}')
        ...
        if mode_t in ('FULL', 'CARCH'):
            for _m in range(m):
                ...
                for _n in range(n):
                    self.cA.iloc[_m][_n] = model1.addVar(lb=0, ub=ub, obj=0, vtype=GRB.INTEGER, name=f'cA_{_m + 1}_{_n + 1}')
                    self.cB.iloc[_m][_n] = model1.addVar(lb=0, ub=ub, obj=0, vtype=GRB.INTEGER, name=f'cB_{_m + 1}_{_n + 1}')
            ...
            if ampdel:
                for _m in range(m):
                    ...
                    if cluster_id not in copy_numbers:
                        adA[_m] = model1.addVar(lb=0, ub=1, obj=0, vtype=GRB.BINARY, name=f'adA_{_m + 1}')
                        adB[_m] = model1.addVar(lb=0, ub=1, obj=0, vtype=GRB.BINARY, name=f'adB_{_m + 1}')
        ...
        if (mode_t == 'FULL') or (d > 0 and mode_t == 'CARCH'):
            for _b in range(_M):
                for _m in range(m):
                    for _n in range(n):
                        bitcA[(_b, _m, _n)] = model1.addVar(lb=0, ub=1, obj=0, vtype=GRB.BINARY,
                                                           name=f'bitcA_{_b + 1}_{_m + 1}_{_n + 1}')
                        bitcB[(_b, _m, _n)] = model1.addVar(lb=0, ub=1, obj=0, vtype=GRB.BINARY,
                                                           name=f'bitcB_{_b + 1}_{_m + 1}_{_n + 1}')
        ...
        if mode_t in ('FULL', 'UARCH'):
            for _n in range(n):
                for _k in range(k):
                    self.u.iloc[_n][_k] = model1.addVar(lb=0, ub=1, obj=0, vtype=GRB.CONTINUOUS, name=f'u_{_n + 1}_{_k + 1}')
        ...
        if mode_t == 'FULL':
            for _b in range(_M):
                for _m in range(m):
                    for _n in range(n):
                        for _k in range(k):
                            vA[(_b, _m, _n, _k)] = model1.addVar(lb=0, ub=1, obj=0, vtype=GRB.CONTINUOUS,
                                                                name=f'vA_{_b + 1}_{_m + 1}_{_n + 1}_{_k + 1}')
                            vB[(_b, _m, _n, _k)] = model1.addVar(lb=0, ub=1, obj=0, vtype=GRB.CONTINUOUS,
                                                                name=f'vB_{_b + 1}_{_m + 1}_{_n + 1}_{_k + 1}')
        ...
        if (mode_t in ('FULL', 'UARCH')) and (self.mu > 0):
            for _n in range(n):
                for _k in range(k):
                    x[(_n, _k)] = model1.addVar(lb=0, ub=1, obj=0, vtype=GRB.BINARY, name=f'x_{_n + 1}_{_k + 1}')
        ...
        if (mode_t in ('FULL', 'CARCH')) and d > 0:
            for _m in range(self.m):
                for _n in range(1, self.n):
                    for _d in range(d):
                        z[(_m, _n, _d)] = model1.addVar(lb=0, ub=1, obj=0, vtype=GRB.BINARY, name=f'z_{_m + 1}_{_n + 1}_{_d + 1}')
        ...
        for _m in range(m):
            ...
            for _k in range(k):
                ...
                model1.addConstr(f_a_values[_k] - _fA <= _yA)
                model1.addConstr(_fA - f_a_values[_k] <= _yA)
                model1.addConstr(f_b_values[_k] - _fB <= _yB)
                model1.addConstr(_fB - f_b_values[_k] <= _yB)
        ...
        if mode_t == 'FULL':
                ...
                for _k in range(k):
                    ...
                    for _n in range(n):
                        for _b in range(_M):
                            ...
                            model1.addConstr(vA[(_b, _m, _n, _k)] <= bitcA[(_b, _m, _n)])
                            model1.addConstr(vA[(_b, _m, _n, _k)] <= self.u.iloc[_n][_k])
                            model1.addConstr(vA[(_b, _m, _n, _k)] >= bitcA[(_b, _m, _n)] + self.u.iloc[_n][_k] - 1)
                    ...
                    model1.addConstr(fA[(_m, _k)] == sum_a)
                ...
                for _k in range(k):
                    ...
                    for _n in range(n):
                        for _b in range(_M):
                            ...
                            model1.addConstr(vB[(_b, _m, _n, _k)] <= bitcB[(_b, _m, _n)])
                            model1.addConstr(vB[(_b, _m, _n, _k)] <= self.u.iloc[_n][_k])
                            model1.addConstr(vB[(_b, _m, _n, _k)] >= bitcB[(_b, _m, _n)] + self.u.iloc[_n][_k] - 1)
                    ...
                    model1.addConstr(fB[(_m, _k)] == sum_b)
            ...
            for _n in range(n):
                for _k in range(k):
                    ...
                    model1.addConstr(_sum >= self.u.iloc[_n][_k])
        ...
        if (mode_t == 'FULL') or (d > 0 and mode_t == 'CARCH'):
            for _m in range(m):
                ...
                for _n in range(n):
                    ...
                    model1.addConstr(self.cA.iloc[_m][_n] == sum_a)
                    model1.addConstr(self.cB.iloc[_m][_n] == sum_b)
                    model1.addConstr(self.cA.iloc[_m][_n] + self.cB.iloc[_m][_n] <= ub)
        ...
        if mode_t == 'CARCH':
            ...
            for _m in range(m):
                for _k in range(k):
                    ...
                    model1.addConstr(fA[(_m, _k)] == _sum)
            ...
            for _m in range(m):
                for _k in range(k):
                    ...
                    model1.addConstr(fB[(_m, _k)] == _sum)
        ...
        if mode_t in ('FULL', 'CARCH'):
            for _m in range(m):
                model1.addConstr(self.cA.iloc[_m][0] == 1)
                model1.addConstr(self.cB.iloc[_m][0] == 1)
            ...
            if ampdel:
                for _m in range(m):
                    ...
                    if cluster_id not in copy_numbers:
                        for _n in range(1, n):
                            model1.addConstr(self.cA.iloc[_m][_n] <= cn_max * adA[_m] + _base - _base * adA[_m])
                            model1.addConstr(self.cA.iloc[_m][_n] >= _base * adA[_m])
                            model1.addConstr(self.cB.iloc[_m][_n] <= cn_max * adB[_m] + _base - _base * adB[_m])
                            model1.addConstr(self.cB.iloc[_m][_n] >= _base * adB[_m])
                    cluster_id = f2_a.index[_m]
                    if cluster_id not in copy_numbers:
                        for _n in range(1, n):
        ...
        if mode_t == 'UARCH':
            ...
            for _m in range(m):
                for _k in range(k):
                    ...
                    _sum2 = 0
                    for _n in range(n):
                        _sum += self._fixed_cA.iloc[_m][_n] * self.u.iloc[_n][_k]
            ...
            for _m in range(m):
                for _k in range(k):
                    ...
                    _sum2 = 0
                    for _n in range(n):
                        _sum += self._fixed_cB.iloc[_m][_n] * self.u.iloc[_n][_k]
        ...
        if mode_t in ('FULL', 'UARCH'):
            for _k in range(k):
                ...
                model1.addConstr(_sum == 1)
                model2.constraints.add(_sum2 == 1)

        if (mode_t in ('FULL', 'UARCH')) and self.mu > 0:
            for _k in range(k):
                for _n in range(1, n):
                    model1.addConstr(x[(_n, _k)] >= self.u.iloc[_n][_k])
                    model1.addConstr(self.u.iloc[_n][_k] >= self.mu * x[(_n, _k)])
                    ...
                    for _d in range(d):
                        for _i in range(1, self.n-1):
                            for _j in range(1, self.n):
                                model1.addConstr(bitcA[(_M, _m, _i)] - bitcA[(_M, _m, _j)] <= 2 - z[(_m, _i, _d)] - z[(_m, _j, _d)])
                                model1.addConstr(bitcA[(_M, _m, _j)] - bitcA[(_M, _m, _i)] <= 2 - z[(_m, _i, _d)] - z[(_m, _j, _d)])
                                model1.addConstr(bitcB[(_M, _m, _i)] - bitcB[(_M, _m, _j)] <= 2 - z[(_m, _i, _d)] - z[(_m, _j, _d)])
                                model1.addConstr(bitcB[(_M, _m, _j)] - bitcB[(_M, _m, _i)] <= 2 - z[(_m, _i, _d)] - z[(_m, _j, _d)])
    ...
    def build_symmetry_breaking(self, model1, model2):
    ...
    def fix_given_cn(self, model1, model2):
        for _m in range(self.m):
            cluster_id = self.f_a.index[_m]
            if cluster_id in self.copy_numbers:
                _cnA, _cnB = self.copy_numbers[cluster_id]
                for _n in range(1, self.n):
                    model1.addConstr(self.cA.iloc[_m][_n] == _cnA)
                    model1.addConstr(self.cB.iloc[_m][_n] == _cnB)
    ...
    def run(self, write_path=None):
        ...
        # The dataframes cA/cB/u contain Gurobi variables with optimized values as the 'X' attribute in each variable
        ...
        def _value_old(var): return getattr(var, 'X', var)
        ...
        assert np.allclose(self.model1.objVal, self.model2.obj())
        assert self.optimized_cA.applymap(_value_old).astype(int).equals(self.optimized2_cA.applymap(_value).astype(int))
        assert self.optimized_cB.applymap(_value_old).astype(int).equals(self.optimized2_cB.applymap(_value).astype(int))
        assert self.optimized_u.applymap(_value_old).equals(self.optimized2_u.applymap(_value))
