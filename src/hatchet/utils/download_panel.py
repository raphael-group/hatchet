...
def main(args=None):
    ...
    if args["refpanel"] == "1000GP_Phase3":
        ...
        sp.download(url=config.urls.onekgp, dirpath=args['refpaneldir'], sentinel_file='1000GP_Phase3.sample')
    else:
        error("Currently, only the 1000 genome panel aligned to GRCh37 without \"chr\" prefix is supported\n", raise_exception=True)
                ...
                for l in f:
                    if l.startswith("chain"):
                        ...
                        if not config.urls.refpanel_genome_chr_notation:
                            l[refpanel_index] = l[refpanel_index].replace('chr', '')
                        if not sample_chr:
                            l[sample_index] = l[sample_index].replace('chr', '')
                        new.write(" ".join(l) + "\n")
                    else:
                        new.write(l)
        return name

    hg38tohg19 = download(url=config.urls.refpanel_hg38tohg19, dirpath=dirpath, overwrite=False, extract=False)
    hg19tohg38 = download(url=config.urls.refpanel_hg19tohg38, dirpath=dirpath, overwrite=False, extract=False)
