...
def main(args=None):
    ...
    plot_2d(bbc=bbc, seg=seg, show_centers=show_centers, xlim=baf_lim, ylim=rdr_lim,
            display=display, outdir=outdir, alpha=alpha)

    plot_1d(bbc=bbc, baf_lim=baf_lim, rdr_lim=rdr_lim, display=display, outdir=outdir, 
            alpha=alpha, show_centromeres=show_centromeres)
    sp.log(" Done plotting\n", level = 'STEP')


def plot_2d(bbc, seg = None, show_centers = False, xlim = None, ylim = None, 
            figsize = (4,4), display = True, outdir = None, alpha = 1):
    """
    ...
    for s, df in bbc.groupby('SAMPLE'):
        ...
        plt.vlines(ymin = ymin, ymax = ymax, x = 0.5, linestyle = ':',
                                 linewidth = 1, colors = 'grey')   

        if show_centers:
            my_seg = seg[seg.SAMPLE == s]
            plt.scatter(my_seg.BAF, my_seg.RD, c = my_seg['#ID'], cmap = mymap, s = 10,
...
def plot_1d(bbc, baf_lim = None, rdr_lim = None, display = False, outdir = None, alpha = 1, show_centromeres = False):
    ...
    for s, df in bbc.groupby('SAMPLE'):
        ...
        plt.subplot(212)
        plot_track(df, chr_ends, chr2centro, yval = 'BAF', ylabel = 'B-allele frequency', 
                   color_field = 'CLUSTER', ylim = baf_lim, display = display, 
                   alpha = alpha, title = s, show_centromeres=show_centromeres)
...
def plot_track(bb, chr_ends, chr2centro, yval = 'RD', ylabel = None, display = True, ylim = None, 
               alpha = 1, color_field = None, title = None, show_centromeres = False):
    """
    ...
    if yval == 'BAF':
        plt.hlines(y = 0.5, xmin = minx, xmax = maxx, colors = 'grey', linestyle = ':',
                   linewidth = 1)
    ...
    if show_centromeres:
        ...
        for i, ch in enumerate(chromosomes):
            ...
            if ch in chr2centro:
                ...
                ax.add_patch(Rectangle(xy=(cent_start + chr_start, miny), 
