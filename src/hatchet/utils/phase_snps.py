...
def main(args=None):
    ...
    if args['refvers'] not in ('hg19', 'hg38'):
        sp.error('The reference genome version of your samples is not "hg19" or "hg38".', raise_exception=True)

    rpd = args['refpaneldir']
    if not os.path.exists(os.path.join(rpd, '1000GP_Phase3.sample')):
        sp.error('Please download the 1000GP reference panel before proceeding', raise_exception=True)
    ...
    if args['refvers'] == 'hg38':
        ...
        sp.ensure(os.path.isfile(chains['hg38_hg19']) and os.path.isfile(chains['hg19_hg38']),
    ...
    phaser = Phaser(panel, outdir=outdir, hg19=hg19_path, ref=args['refgenome'], chains=chains,
                    rename=rename_files, refvers=args['refvers'], chrnot=args['chrnot'], verbose=False,
                    bcftools=bcftools, shapeit=shapeit, picard=picard, bgzip=bgzip)
...
class Phaser(Worker):
    ...
    def __init__(self, panel, outdir, hg19, ref, chains, rename, refvers, chrnot, verbose, bcftools, shapeit, picard,
                 bgzip):
        self.panel = panel
        self.outdir = outdir
        self.hg19 = hg19
    ...
    def work(self, *args):
        ...
        if self.refvers == 'hg19':
            ...
            if self.chrnot:
                vcf_toFilter = self.change_chr(infile=vcf, chromosome=chromosome, outname='toFilter',
            ...
            # liftover
            vcf_toFilter = self.liftover(infile=vcf, chromosome=chromosome, outname='toFilter',
                                         chain=self.chains['hg38_hg19'], refgen=self.hg19, ch=False)
        ...
        if self.refvers == 'hg19':
            if self.chrnot:
                vcf_to_concat = self.change_chr(infile=vcf_phased, chromosome=chromosome, outname='toConcat',
            ...
            vcf_to_concat = self.liftover(infile=vcf_phased, chromosome=chromosome, outname='toConcat',
                                          chain=self.chains['hg19_hg38'], refgen=self.ref, ch=self.chrnot)
    ...
    def liftover(self, infile, chromosome, outname, chain, refgen, ch):
        ...
        if any(c != 0 for c in codes) or pic.returncode != 0:
            ...
            sp.error(f'Failed to liftover chromosomes with picard on {infile}.', raise_exception=True)
        else:
            os.remove(errname)
        return outfile
    ...
    def run_shapeit(self, infile, chromosome):
        ...
        cmd_check = f'{self.shapeit} -check --input-vcf {self.outdir}/{chromosome}_filtered.vcf.gz ' \
                    f'--input-map {inmap} ' \
                    f'--input-ref {inref} ' \
                    f'--output-log {self.outdir}/{chromosome}_alignments'

        cmd_phase = f'{self.shapeit} --input-vcf {self.outdir}/{chromosome}_filtered.vcf.gz ' \
                    f'--input-map {inmap} ' \
                    f'--input-ref {inref} ' \
        ...
        cmd_convert = f'{self.shapeit} -convert --input-haps {self.outdir}/{chromosome} ' \
        ...
        sp.run(cmd_check, stdouterr_filepath=errname, check_return_codes=False)  # May return 1
        sp.run([cmd_phase, cmd_convert, cmd_compress, cmd_index], stdouterr_filepath=errname,
