...
def generate_1D2D_plots(
    ...
    if any(using_chr):
        if not all(using_chr):
            raise ValueError(
                error("Some chromosomes use 'chr' notation while others do not.")
            )
        use_chr = True
    else:
        use_chr = False
    ...
    _, mapping = reindex(
        [k for k, _ in bbc.groupby([f"cn_clone{i + 1}" for i in range(n_clones)])]
    )
...
def limits_valid(lim):
    return (
        lim is not None and len(lim) == 2 and lim[0] is not None and lim[1] is not None
    )
        ...
        if coloring == "local":
            ...
            for i in present_clones:
                if i == 0:
                    ...
                    legendstr.append(
                        "Clone {}: {:.3f}".format(i, bbc_["u_clone{}".format(i)][0])
                    )
