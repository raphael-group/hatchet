...
def main(args=None):
    ...
    if args['regions'] is None:
        log(
            msg='# Retrieving genomic regions to consider from maximum chromosome length\n',
            level='STEP',
        )
        regions = knownRegions(args['refdict'], args['chromosomes'])
    else:
        log(
            msg='# Checking the consistency of the given regions\n',
            level='STEP',
        )
    ...
    if not normal_bins:
        close('No bins in the normal sample!\n')
    ...
    if not tumor_bins:
        close('No bins in the tumor samples!\n')
    ...
    total_counts = tc.tcount(
        samtools=args['samtools'],
        samples=({args['normal']} | args['samples']),
        chromosomes=args['chromosomes'],
        num_workers=args['j'],
        q=args['q'],
        verbose=args['verbose'],
