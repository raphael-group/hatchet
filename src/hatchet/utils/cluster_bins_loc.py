...
def main(args=None):
    ...
    args = parse_cluster_bins_loc_args(args)
    sp.logArgs(args, 80)
    ...
    while chr_idx < len(tracks):
        ...
        assert tracks[chr_idx].shape[1] == i - start_idx, (tracks[chr_idx].shape[1], i - start_idx)
...
def hmm_model_select(tracks, minK = 20, maxK = 50, tau = 10e-6, tmat = 'diag', decode_alg = 'viterbi', covar = 'diag', seed = 0):
    ...
    for K in range(minK, maxK + 1):
        ...
        if tmat == 'fixed':
            model = hmm.GaussianHMM(n_components = K, init_params = 'mc', params = 'smc', covariance_type = covar, random_state = 0)
        elif tmat == 'free':
            model = hmm.GaussianHMM(n_components = K, init_params = 'mc', params = 'smct', covariance_type = covar, random_state = 0)
        else:
            model = DiagGHMM(n_components = K, init_params = 'mc', params = 'smct', covariance_type = covar, random_state = 0)
