...
def rd_gccorrect(bb, ref_genome):
    ...
    bb['GCCORR'] = bb.groupby('SAMPLE')['GC'].transform(lambda X: gccorrect[X.name].predict(X.to_frame('GC')).values)
    bb['RD'] = bb['UNCORR_RD'] / bb['GCCORR'].where((bb['GCCORR'] > 0) & ~pd.isnull(bb['GCCORR']), 1)
