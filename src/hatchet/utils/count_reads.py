...
def main(args=None):
    ...
    if os.path.exists(totals_file):
        ...
        try:
            total = {
                name: sum(total_counts[name, chromosome] for chromosome in chromosomes)
                for name in names
            }
        except KeyError:
            raise KeyError(
...
def count_chromosome(
    ...
    try:
        ...
        if st.returncode != 0:
            raise ValueError(
                "samtools subprocess returned nonzero value: {}".format(st.returncode)
            )
        if cut.returncode != 0:
            raise ValueError(
                "cut subprocess returned nonzero value: {}".format(cut.returncode)
            )
...
def read_snps(baf_file, ch, all_names):
    """
    Read and validate SNP data for this patient.
    """
    all_names = all_names[
        1:
...
def run_chromosome(
    ...
    try:
        ...
        perpos_files = [
            os.path.join(outdir, name + ".per-base.bed.gz") for name in all_names
        ]
