...
def parse_plot_bins_1d2d_args(args=None):
    ...
    """
    description = ""
    parser = argparse.ArgumentParser(prog='hatchet plot-cn', description=description,
    ...
    parser.add_argument("-s", "--seg",
                        ...
                        required=False, type=str, default=config.plot_bins_1d2d.seg)
    parser.add_argument("-O", "--outdir", required=True, type=str, help='Directory for output files')
    parser.add_argument("--baflim", required=False, type=str,
    ...
    parser.add_argument("--rdrlim", required=False, type=str,
    ...
    parser.add_argument("--centers", required=False, action='store_true',
                        help="Show cluster centers (requires SEG file provided via -s/--seg)",
                        default=config.plot_bins_1d2d.centers)
    parser.add_argument("--centromeres", required=False, action='store_true',
                        help="Mark centromere locations with grey rectangles",
                        default=config.plot_bins_1d2d.centromeres)
    parser.add_argument("-a", "--alpha", required=False, type=float, help="Opacity alpha (default 1)",
...
def parse_plot_cn_1d2d_args(args=None):
    ...
    parser.add_argument("--fcnlim", required=False, type=str,
    ...
    parser.add_argument("--centromeres", required=False, action='store_true',
                        help="Mark centromere locations with grey rectangles", default=config.plot_cn_1d2d.centromeres)
    parser.add_argument("--bysample", required=False, action='store_true',
...
def parse_cluster_bins_loc_args(args=None):
    ...
    parser = argparse.ArgumentParser(prog='hatchet cluster-bins', description=description,
                                     formatter_class=argparse.RawTextHelpFormatter)
    parser.add_argument("BBFILE",
    ...
    parser.add_argument("-o", "--outsegments", required=False, default=config.cluster_bins_loc.outsegments, type=str,
                        help="Output filename for the segments computed by clustering bins (default: stdout)")
    parser.add_argument("-O", "--outbins", required=False, default=config.cluster_bins_loc.outbins, type=str,
                        help="Output filename for a BB file adding the clusters (default: stdout)")
    parser.add_argument("-d", "--diploidbaf", type=float, required=False, default=config.cluster_bins_loc.diploidbaf,
    ...
    parser.add_argument("-e", "--seed", type=int, required=False, default=config.cluster_bins_loc.seed,
                        help='Random seed used for clustering (default: 0)')
    parser.add_argument("--minK", type=int, required=False, default=config.cluster_bins_loc.mink,
                        help=f'Minimum number of clusters to infer (default = {config.cluster_bins_loc.mink})')
    parser.add_argument("--maxK", type=int, required=False, default=config.cluster_bins_loc.maxk,
                        help=f'Maximum number of clusters to infer (default = {config.cluster_bins_loc.maxk})')
    parser.add_argument("--exactK", type=int, required=False, default=config.cluster_bins_loc.exactk,
                        help='Skip model selection and infer exactly this many clusters (default: None)')
    parser.add_argument("-t", "--transmat", type=str, required=False, default=config.cluster_bins_loc.transmat,
                        help="Form of transition matrix to infer: fixed, diag (1-parameter), or full (default: diag)")
    parser.add_argument("--tau", type=float, required=False, default=config.cluster_bins_loc.tau,
                        help=f"Off-diagonal value for initializing transition matrix (default: {config.cluster_bins_loc.tau})")
    parser.add_argument("-c", "--covar", type=str, required=False, default=config.cluster_bins_loc.covar,
                        help="Form of covariance matrix: spherical, diag, full, or tied (default: diag)")
    parser.add_argument("-x", "--decoding", type=str, required=False, default=config.cluster_bins_loc.decoding,
    ...
    ensure(args.minK <= args.maxK, 'Min number of clusters must be smaller than max number of clusters.')
    ...
    ensure(args.covar in valid_covars, f'Invalid -c/--covar argument: {args.covar}. Valid values are {valid_covars}.')

    valid_decodings = ('map', 'viterbi')
    ensure(args.decoding in valid_decodings, f'Invalid -x/--decoding argument: {args.decoding}. Valid values are {valid_decodings}.')

    valid_transmats = ('fixed', 'diag', 'tied')
    ensure(args.transmat in valid_transmats, f'Invalid -t/--transmat argument: {args.transmat}. Valid values are {valid_transmats}.')
...
def parse_count_reads_args(args=None):
    ...
    parser.add_argument("-N", "--normal", required=True, type=str,
                        help="BAM file corresponding to matched normal sample")
    parser.add_argument("-T", "--tumor", required=True, type=str, nargs='+',
                        help="BAM files corresponding to samples from the same tumor")
    parser.add_argument("-b", "--baffile", required=True, type=str,
                        help="1bed file containing SNP information from tumor samples (i.e., baf/bulk.1bed)")
    parser.add_argument("-V", "--refversion", required=True, type=str,
                        help="Version of reference genome used in BAM files")
    parser.add_argument("-O", "--outdir", required=True, type=str, help='Directory for output files')
    parser.add_argument("-S", "--samples", required=False, default=config.count_reads.samples, type=str, nargs='+',
                        help="Sample names for each BAM, given in the same order where the normal name is first (default: inferred from file names)")
    parser.add_argument("-st", "--samtools", required=False, type=str, default=config.paths.samtools,
                        help='Path to samtools executable')
    parser.add_argument("-md", "--mosdepth", required=False, type=str, default=config.paths.mosdepth,
                        help='Path to mosdepth executable')
    parser.add_argument("-tx", "--tabix", required=False, type=str, default=config.paths.tabix,
                        help='Path to tabix executable')
    parser.add_argument("-j", "--processes", required=False, default=config.count_reads.processes, type=int,
                        help="Number of available parallel processes (default: 2)")
    parser.add_argument("-q", "--readquality", required=False, default=config.count_reads.readquality, type=int,
                        help=f"Minimum mapping quality for an aligned read to be considered (default: {config.count_reads.readquality})")
    parser.add_argument("-i", "--intermediates", required=False, action='store_true',
    ...
    ver = args.refversion
    ensure(ver in ('hg19', 'hg38'), 'Invalid reference genome version. Supported versions are hg38 and hg19.')
    ...
    ensure(os.path.exists(args.outdir), f'Output directory <{args.outdir}> not found!')
...
def parse_combine_counts_args(args=None):
    ...
    parser.add_argument('-A', '--array', type=str, required=True,
                        help=f'Directory containing array files (output from "count_reads" command)')
    parser.add_argument("-t", "--totalcounts", required=True, type=str,
    ...
    parser.add_argument('--msr', type=int, help=f'Minimum SNP reads per bin (default {config.combine_counts.msr})',
                        default=config.combine_counts.msr)
    parser.add_argument('--mtr', type=int, help=f'Minimum total reads per bin (default {config.combine_counts.mtr})',
    ...
    parser.add_argument("-p", "--phase", required=False, default=config.combine_counts.phase, type=str,
                        help='VCF file containing phasing for heterozygous germline SNPs')
    parser.add_argument("-s", "--max_blocksize", required=False, default=config.combine_counts.blocksize, type=int,
                        help=f'Maximum size of phasing block (default {config.combine_counts.blocksize})')
    parser.add_argument("-m", "--max_spb", required=False, default=config.combine_counts.max_spb, type=int,
                        help=f'Maximum number of SNPs per phasing block (default {config.combine_counts.max_spb})')
    parser.add_argument("-a", "--alpha", required=False, default=config.combine_counts.alpha, type=float,
    ...
    parser.add_argument("--ss_em", action='store_true', default=config.combine_counts.ss_em, required=False,
                        help="Use single-sample EM BAF inference (instead of multi-sample)")
    parser.add_argument("-z", '--not_compressed', action='store_true', default=config.combine_counts.not_compressed,
                        required=False, help="Non-compressed intermediate files")
    parser.add_argument("-V", "--refversion", required=True, type=str,
                        help="Version of reference genome used in BAM files")
    args = parser.parse_args(args)
    ...
    ensure(0 <= args.alpha <= 1, 'The alpha argument must be between 0 and 1, inclusive.')
    ensure(os.path.exists(args.array), f'The provided array directory does not exist: {args.array}')

    namesfile = os.path.join(args.array, 'samples.txt')
    ensure(os.path.exists(namesfile), 'Missing file containing sample names (1 per line): {namesfile}')
    ...
    ensure(args.msr > 0, 'The minimum number of SNP-covering reads must be positive.')
    ...
    ensure(os.path.exists(outdir), f'Directory for output file does not exist: <{outdir}>')
...
def parse_genotype_snps_arguments(args=None):
    ...
    parser.add_argument("-bt", "--bcftools", required=False, default=config.paths.bcftools, type=str,
    ...
    parser.add_argument("-R", "--snps", required=False, default=config.genotype_snps.snps, type=str,
                        help="List of SNPs to consider in the normal sample (default: heterozygous SNPs are inferred from the normal sample)")
    parser.add_argument("-j", "--processes", required=False, default=config.genotype_snps.processes, type=int,
                        help="Number of available parallel processes (default: 2)")
    parser.add_argument("-q", "--readquality", required=False, default=config.genotype_snps.readquality, type=int,
                        help="Minimum mapping quality for an aligned read to be considered (default: 0)")
    parser.add_argument("-Q", "--basequality", required=False, default=config.genotype_snps.basequality, type=int,
                        help="Minimum base quality for a base to be considered (default: 11)")
    parser.add_argument("-c", "--mincov", required=False, default=config.genotype_snps.mincov, type=int,
                        help="Minimum coverage for SNPs to be considered (default: 0)")
    parser.add_argument("-C", "--maxcov", required=False, default=config.genotype_snps.maxcov, type=int,
    ...
    parser.add_argument("-E", "--newbaq", required=False, action='store_true', default=config.genotype_snps.newbaq,
                        help="Recompute alignment of reads on the fly during SNP calling (default: false)")
    parser.add_argument("-o", "--outputsnps", required=False, default=config.genotype_snps.outputsnps, type=str,
                        help="Output folder for SNPs separated by chromosome (default: ./)")
    parser.add_argument("-v", "--verbose", action='store_true', default=config.genotype_snps.verbose, required=False,
...
def parse_download_panel_arguments(args=None):
    ...
    parser.add_argument("-D", "--refpaneldir", required=False, type=str, default=config.download_panel.refpaneldir,
                        help="Path to Reference Panel")
    parser.add_argument("-R", "--refpanel", default=config.download_panel.refpanel, required=False, type=str,
...
def parse_phase_snps_arguments(args=None):
    ...
    parser.add_argument("-V", "--refversion", required=True, type=str,
                        help="Version of reference genome used in BAM files")
    parser.add_argument("-N", "--chrnotation", required=True, action='store_true',
    ...
    parser.add_argument("-L", "--snps", required=True, type=str, nargs='+',
                        help="List of SNPs in the normal sample to phase")
    parser.add_argument("-j", "--processes", required=False, default=config.genotype_snps.processes, type=int,
                        help="Number of available parallel processes (default: 2)")
    parser.add_argument("-si", "--shapeit", required=False, type=str, default=config.paths.shapeit,
                        help='Path to shapeit executable (default: look in $PATH)')
    parser.add_argument("-pc", "--picard", required=False, type=str, default=config.paths.picard,
    ...
    parser.add_argument("-bg", "--bgzip", required=False, default=config.paths.bgzip, type=str,
                        help="Path to the directory of \"bcftools\" executable (default: look in $PATH)")
    args = parser.parse_args(args)
...
def parse_count_alleles_arguments(args=None):
    ...
    parser.add_argument("-N", "--normal", required=True, type=str,
                        help="BAM file corresponding to matched normal sample")
    parser.add_argument("-T", "--tumors", required=True, type=str, nargs='+',
                        help="BAM files corresponding to samples from the same tumor")
    parser.add_argument("-r", "--reference", required=True, type=str, help="Human reference genome of BAMs")
    parser.add_argument("-L", "--snps", required=True, type=str, nargs='+',
                        help="List of SNPs to consider in the normal sample")
    parser.add_argument("-S", "--samples", required=False, default=config.count_alleles.samples, type=str, nargs='+',
                        help="Sample names for each BAM (given in the same order where the normal name is first)")
    parser.add_argument("-st", "--samtools", required=False, default=config.paths.samtools, type=str,
                        help="Path to the directory to \"samtools\" executable, required in default mode (default: samtools is directly called as it is in user $PATH)")
    parser.add_argument("-bt", "--bcftools", required=False, default=config.paths.bcftools, type=str,
                        help="Path to the directory of \"bcftools\" executable, required in default mode (default: bcftools is directly called as it is in user $PATH)")
    parser.add_argument("-e", "--regions", required=False, default=config.count_alleles.regions, type=str,
    ...
    parser.add_argument("-j", "--processes", required=False, default=config.count_alleles.processes, type=int,
                        help="Number of available parallel processes (default: 2)")
    parser.add_argument("-q", "--readquality", required=False, default=config.count_alleles.readquality, type=int,
                        help="Minimum mapping quality for an aligned read to be considered (default: 0)")
    parser.add_argument("-Q", "--basequality", required=False, default=config.count_alleles.basequality, type=int,
                        help="Minimum base quality for a base to be considered (default: 11)")
    parser.add_argument("-U", "--snpquality", required=False, default=config.count_alleles.snpquality, type=int,
                        help="Minimum SNP-variant quality, QUAL, for a variant to be considered (default: 11)")
    parser.add_argument("-g", "--gamma", required=False, default=config.count_alleles.gamma, type=float,
                        help="Level of confidence to determine heterozigosity of SNPs (default: 0.05)")
    parser.add_argument("-b", "--maxshift", required=False, default=config.count_alleles.maxshift, type=float,
                        help="Maximum allowed absolute difference of BAF from 0.5 for selected heterozygous SNPs in the normal sample (default: 0.5)")
    parser.add_argument("-c", "--mincov", required=False, default=config.count_alleles.mincov, type=int,
                        help="Minimum coverage for SNPs to be considered (default: 0)")
    parser.add_argument("-C", "--maxcov", required=False, default=config.count_alleles.maxcov, type=int,
                        help="Maximum coverage for SNPs to be considered (default: 1000, suggested: twice the values of expected average coverage to avoid aligning artefacts)")
    parser.add_argument("-E", "--newbaq", required=False, action='store_true', default=config.count_alleles.newbaq,
                        help="Recompute alignment of reads on the fly during SNP calling (default: false)")
    parser.add_argument("-O", "--outputnormal", required=False, default=config.count_alleles.outputnormal, type=str,
                        help="Filename of output for allele counts in the normal sample (default: standard output)")
    parser.add_argument("-o", "--outputtumors", required=False, default=config.count_alleles.outputtumors, type=str,
                        help="Output filename for allele counts in tumor samples (default: standard output)")
    parser.add_argument("-l", "--outputsnps", required=False, default=config.count_alleles.outputsnps, type=str,
                        help="Output directory for lists of selected SNPs (default: ./)")
    parser.add_argument("-v", "--verbose", action='store_true', default=config.count_alleles.verbose, required=False,
                        help="Use verbose log messages")
    parser.add_argument("-V", "--version", action='version', version=f'%(prog)s {__version__}')
    args = parser.parse_args(args)
    ...
    ensure(0 <= args.maxshift <= 0.5, 'Max BAF shift must be a floating value between 0 and 0.5')
    ensure(args.mincov >= 0, 'The minimum-coverage value must be positive')
    ensure(args.maxcov >= 0, 'The maximum-coverage value must be positive')
...
def parse_count_reads_fw_arguments(args=None):
    ...
    parser.add_argument("-T", "--tumors", required=True, type=str, nargs='+',
                        help="BAM files corresponding to samples from the same tumor")
    parser.add_argument("-b", "--size", required=True, type=str,
                        help="Size of the bins, specified as a full number or using the notations either \"kb\" or \"Mb\"")
    parser.add_argument("-S", "--samples", required=False, default=config.count_reads_fw.samples, type=str, nargs='+',
                        help="Sample names for each BAM, given in the same order where the normal name is first (default: inferred from file names)")
    parser.add_argument("-st", "--samtools", required=False, default=config.paths.samtools, type=str,
                        help="Path to the directory to \"samtools\" executable, required in default mode (default: samtools is directly called as it is in user $PATH)")
    parser.add_argument("-r", "--regions", required=False, default=config.count_reads_fw.regions, type=str,
    ...
    parser.add_argument("-g", "--reference", required=False, default=config.paths.reference, type=str,
    ...
    parser.add_argument("-j", "--processes", required=False, default=config.count_reads_fw.processes, type=int,
                        help="Number of available parallel processes (default: 2)")
    parser.add_argument("-q", "--readquality", required=False, default=config.count_reads_fw.readquality, type=int,
                        help=f"Minimum mapping quality for an aligned read to be considered (default: {config.count_reads_fw.readquality})")
    parser.add_argument("-O", "--outputnormal", required=False, default=config.count_reads_fw.outputnormal, type=str,
                        help="Filename of output for allele counts in the normal sample (default: standard output)")
    parser.add_argument("-o", "--outputtumors", required=False, default=config.count_reads_fw.outputtumors, type=str,
                        help="Output filename for allele counts in tumor samples (default: standard output)")
    parser.add_argument("-t", "--outputtotal", required=False, default=config.count_reads_fw.outputtotal, type=str,
                        help="Output filename for total read counts in all tumor samples (default: \"total_read.counts\")")
    parser.add_argument("-v", "--verbose", action='store_true', default=config.count_reads_fw.verbose, required=False,
                        help="Use verbose log messages")
    parser.add_argument("-V", "--version", action='version', version=f'%(prog)s {__version__}')
    args = parser.parse_args(args)
...
def parse_combine_counts_fw_args(args=None):
    ...
    parser = argparse.ArgumentParser(prog='hatchet combine-counts', description=description,
                                     formatter_class=argparse.RawTextHelpFormatter)
    parser.add_argument("-c", "--normalbins", required=True, type=str,
                        help='Normal bin counts in the format "SAMPLE\tCHR\tSTART\tEND\tCOUNT"')
    parser.add_argument("-C", "--tumorbins", required=True, type=str,
                        help='Tumor bin counts in the format "SAMPLE\tCHR\tSTART\tEND\tCOUNT"')
    parser.add_argument("-B", "--tumorbafs", required=True, type=str,
    ...
    parser.add_argument("-d", "--diploidbaf", type=float, required=False, default=0.1,
    ...
    parser.add_argument("-l", "--blocklength", required=False, default=config.combine_counts.blocksize, type=str,
                        help="Size of the haplotype blocks, specified as a full number or using the notations either \"kb\" or \"Mb\" (default: 50kb)")
    parser.add_argument("-t", "--totalcounts", required=False, default=None, type=str,
    ...
    parser.add_argument("-g", "--gamma", type=float, required=False, default=0.05,
                        help='Confidence level used to determine if a bin is copy neutral with BAF of 0.5 in the BINOMIAL_TEST mode (default: 0.05)')
    parser.add_argument("-e", "--seed", type=int, required=False, default=0,
...
def parse_cluster_bins_args(args=None):
    ...
    parser.add_argument("BBFILE",
                        help="A BB file containing a line for each bin in each sample and the corresponding values of read-depth ratio and B-allele frequency (BAF)")
    parser.add_argument("-o", "--outsegments", required=False, default=config.cluster_bins.outsegments, type=str,
                        help="Output filename for the segments computed by clustering bins (default: stdout)")
    parser.add_argument("-O", "--outbins", required=False, default=config.cluster_bins.outbins, type=str,
                        help="Output filename for a BB file adding the clusters (default: stdout)")
    parser.add_argument("-d", "--diploidbaf", type=float, required=False, default=config.cluster_bins.diploidbaf,
                        help="Maximum diploid-BAF shift used to determine the largest copy-neutral cluster and to rescale all the cluster inside this threshold accordingly (default: None, scaling is not performed)")
    parser.add_argument("-tR", "--tolerancerdr", type=float, required=False, default=config.cluster_bins.tolerancerdr,
    ...
    parser.add_argument("-tB", "--tolerancebaf", type=float, required=False, default=config.cluster_bins.tolerancebaf,
    ...
    parser.add_argument("-u", "--bootclustering", type=int, required=False, default=config.cluster_bins.bootclustering,
    ...
    parser.add_argument("-dR", "--ratiodeviation", type=float, required=False,
    ...
    parser.add_argument("-dB", "--bafdeviation", type=float, required=False, default=config.cluster_bins.bafdeviation,
                        help='Standard deviation of the BAFs used to generate the points in the clouds (default: 0.02)')
    parser.add_argument("-e", "--seed", type=int, required=False, default=config.cluster_bins.seed,
                        help='Random seed used for clustering AND the normal distributions used in the clouds (default: 0)')
    parser.add_argument("-K", "--initclusters", type=int, required=False, default=config.cluster_bins.initclusters,
                        help="The maximum number of clusters to infer (default: 50)")
    parser.add_argument("-c", "--concentration", type=float, required=False, default=config.cluster_bins.concentration,
    ...
    parser.add_argument("-R", "--restarts", type=int, required=False, default=config.cluster_bins.restarts,
                        help="Number of restarts performed by the clustering to choose the best (default: 10)")
    parser.add_argument("-v", "--verbose", action='store_true', default=config.cluster_bins.verbose, required=False,
                        help="Use verbose log messages")
    parser.add_argument("--disablebar", action='store_true', default=config.cluster_bins.disablebar, required=False,
...
def parse_plot_bins_args(args=None):
    ...
    parser = argparse.ArgumentParser(prog='hatchet plot-bins', description=description,
                                     formatter_class=argparse.RawTextHelpFormatter)
    parser.add_argument("INPUT", help='Input BBC file with RDR and BAF')
    parser.add_argument("-c", "--command", required=False, type=str, default=config.plot_bins.command,
                        help="The command determining the plots to generate (default: all)\n\n\t{}RD{}: Plot the read-depth ratio (RD) values of the genomes for each sample.\n\n\t{}CRD{}: Plot the read-depth ratio (CRD) values of the genomes for each sample colored by corresponding cluster.\n\n\t{}BAF{}: Plot the B-allele frequency (BAF) values of the genomes for each sample.\n\n\t{}CBAF{}: Plot BAF values for each sample colored by corresponding cluster.\n\n\t{}BB{}: Plot jointly the values of read-depth ratio (RD) and B-allele frequency (BAF) for each bin in all samples and their density.\n\n\t{}CBB{}: Plot jointly the values of read-depth ratio (RD) and B-allele frequency (BAF) for each bin in all samples by coloring the bins depending on their cluster.\n\n\t{}CLUSTER{}: Plot jointly the values of read-depth ratio (RD) and B-allele frequency (BAF) for each cluster in all samples where the size of the markers is proportional to the number of bins.".format(
                            sp.bcolors.BOLD, sp.bcolors.ENDC, sp.bcolors.BOLD, sp.bcolors.ENDC, sp.bcolors.BOLD,
                            sp.bcolors.ENDC, sp.bcolors.BOLD, sp.bcolors.ENDC, sp.bcolors.BOLD, sp.bcolors.ENDC,
                            sp.bcolors.BOLD, sp.bcolors.ENDC, sp.bcolors.BOLD, sp.bcolors.ENDC, sp.bcolors.BOLD,
                            sp.bcolors.ENDC))
    parser.add_argument("-s", "--segfile", required=False, type=str, default=config.plot_bins.segfile,
                        help="When the corresponding seg file is provided the clusters are also plotted (default: none)")
    parser.add_argument("-m", "--colormap", required=False, type=str, default=config.plot_bins.colormap,
    ...
    parser.add_argument("-tC", "--chrthreshold", required=False, type=int, default=config.plot_bins.chrthreshold,
                        help='Only covering at least this number of chromosomes are considered (default: None)')
    parser.add_argument("-tS", "--sizethreshold", required=False, type=float, default=config.plot_bins.sizethreshold,
                        help='Only covering at least this genome proportion (default: None)')
    parser.add_argument("--resolution", required=False, default=config.plot_bins.resolution, type=int,
                        help='Resolution of bins (default: bins are not merged)')
    parser.add_argument("--xmin", required=False, default=config.plot_bins.xmin, type=float,
                        help='Minimum value on x-axis for supported plots (default: inferred from data)')
    parser.add_argument("--xmax", required=False, default=config.plot_bins.xmax, type=float,
                        help='Maximum value on x-axis for supported plots (default: inferred from data)')
    parser.add_argument("--ymin", required=False, default=config.plot_bins.ymin, type=float,
                        help='Minimum value on y-axis for supported plots (default: inferred from data)')
    parser.add_argument("--ymax", required=False, default=config.plot_bins.ymax, type=float,
                        help='Maximum value on y-axis for supported plots (default: inferred from data)')
    parser.add_argument("--figsize", required=False, default=config.plot_bins.figsize, type=str,
                        help='Size of the plotted figures in the form "(X-SIZE, Y-SIZE)"')
    parser.add_argument("--markersize", required=False, default=config.plot_bins.markersize, type=int,
                        help='Size of the markers (default: values inferred for each plot)')
    parser.add_argument("--colwrap", required=False, default=config.plot_bins.colwrap, type=int,
                        help='Wrapping the plots in this number of columnes (default: 2)')
    parser.add_argument("--fontscale", required=False, default=config.plot_bins.fontscale, type=float,
                        help='Font scale (default: 1)')
    parser.add_argument("-x", "--rundir", required=False, default=config.plot_bins.rundir, type=str,
                        help='Running dirrectory where output the results (default: current directory)')
    parser.add_argument("--pdf", action='store_true', default=config.plot_bins.pdf, required=False,
                        help="Output the bb_clustered figure in PDF format (default: PNG)")
    parser.add_argument("--dpi", required=False, default=config.plot_bins.dpi, type=int,
...
def extractChromosomes(samtools, normal, tumors, reference=None):
    ...
    for tumor in tumors:
        ...
        if not chromosomes <= tumor_sq:
            sys.stderr.write(
                "WARNING: chromosomes {} are not present in the tumor sample {}\n".format(chromosomes - tumor_sq,
                                                                                          tumor))

    # Check consistency of chromosome names with the reference
    if reference is not None:
        stdout, stderr = subprocess.Popen("grep -e \"^>\" {}".format(reference), stdout=subprocess.PIPE, shell=True,
...
def getSQNames(samtools, bamfile):
    header, stderr = subprocess.Popen([samtools, "view", "-H", bamfile], stdout=subprocess.PIPE, shell=False,
                                      universal_newlines=True).communicate()
    if stderr is not None:
