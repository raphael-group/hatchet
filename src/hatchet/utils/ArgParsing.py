...
def parse_plot_cn_1d2d_args(args=None):
    ...
    parser.add_argument(
        "INPUT", help="Filename for BBC table (e.g., results/best.bbc.ucn)"
    )
    parser.add_argument(
        "-O",
        "--outdir",
...
def parse_cluster_bins_args(args=None):
    ...
    parser.add_argument(
        ...
        nargs="+",
        help=(
            "List of sample names to use as a subset of those included in binning"
            "(default: none, run on all samples)"
        ),
    )
    parser.add_argument(
        "--allow_gaps",
...
def parse_count_reads_args(args=None):
    ...
    chromosomes = extractChromosomes(
        samtools, [bams[0], "normal"], [(x, "") for x in bams[1:]]
    )
    if args.chromosomes:
...
def parse_combine_counts_args(args=None):
    ...
    parser.add_argument(
        "-o", "--outfile", required=True, type=str, help="Filename for output"
    )
    parser.add_argument(
        "--msr",
        type=int,
...
def parse_genotype_snps_arguments(args=None):
    ...
    parser = argparse.ArgumentParser(
        prog="hatchet genotype-snps", description=description
    )
    parser.add_argument(
        "-N",
        "--normal",
    ...
    if (
        ...
        and isfile(args.snps)
        and args.snps.endswith("gz")
        and not isfile(args.snps + ".tbi")
    ):
        error(
            "The provided list of SNPs is a bgzip compressed vcf file"
            "but the associated tabix file does not exist!",
            raise_exception=True,
        )
...
def parse_download_panel_arguments(args=None):
    description = (
        "Download and prepare files for phasing germline SNPs using a reference panel"
    )
...
def parse_phase_snps_arguments(args=None):
    ...
    for f in args.snps:
        ...
        snplists = {
            os.path.basename(f).split(".")[0].replace("chr", ""): f for f in args.snps
        }
    ...
    chromosomes = [
        os.path.basename(snplists[i]).replace(".vcf.gz", "").replace("chr", "")
        for i in snplists
    ]
...
def parse_count_alleles_arguments(args=None):
    ...
    if not isfile(args.reference):
        raise ValueError(
            error("The provided file for human reference genome does not exist!")
        )
...
def getSQNames(samtools, bamfile):
    ...
    if stderr is not None:
        raise ValueError(
            "The header of the normal-sample BAM cannot be read with samtools!"
        )
    names = set()
...
def parseRegions(region_file, chromosomes):
    ...
    for key in res:
        ...
        if not all(
            a[0] <= a[1] <= b[0] <= b[1] for a, b in zip(res[key], res[key][1:])
        ):
            error(
                (
