...
def parse_clubb_args(args=None):
    ...
    parser.add_argument("-e","--seed", type=int, required=False, default=config.clubb.seed, help='Random seed used for the normal distributions used in the clouds (default: 0)')
    parser.add_argument("-K","--initclusters", type=int, required=False, default=config.clubb.initclusters, help="The initial number of clusters (default: 15)")
    parser.add_argument("-sf","--tuning", type=float, required=False, default=config.clubb.tuning, help="Tuning parameter for clustering; used to determine initial size of distribution covariances. Small sf indicates a belief that clusters are of small size (default: 0.01)")
