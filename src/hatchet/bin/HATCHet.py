...
def parsing_arguments(args=None):
    ...
    parser.add_argument("-f","--noampdel", action='store_true', default=config.compute_cn.noampdel, required=False, help="Remove amp-del assumption where each mutated allele of every segment can be either amplified or deleted in all tumor clones w.r.t. base (2 for diploid and 4 for tetraploid) (default: use assumption)")
    parser.add_argument("-c","--clonal", type=str, required=False, default=config.compute_cn.clonal, help="Clonal clusters to fix for tetraploid (default: automatically inferred)")
    parser.add_argument("-d","--cnstates", type=int, required=False, default=config.compute_cn.cnstates, help="Maximum number of distinct copy-number states for each segment (default: None, no limit)")
    parser.add_argument("-eD","--diploidcmax", type=int, required=False, default=config.compute_cn.diploidcmax, help="Maximum copy-number value overall segments (default: 6, 0 means inferred from scaled fractional copy numbers)")
    parser.add_argument("-eT","--tetraploidcmax", type=int, required=False, default=config.compute_cn.tetraploidcmax, help="Maximum copy-number value overall segments (default: 12, 0 means inferred from scaled fractional copy numbers)")
    parser.add_argument("-ts","--minsize", type=float, required=False, default=config.compute_cn.minsize, help="The minimum proportion of covered genome for potential clonal clusters (default: 0.008)")
    parser.add_argument("-tc","--minchrs", type=int, required=False, default=config.compute_cn.minchrs, help="The minimum number of covered chromosomes for potential clonal clusters (default: 1)")
    parser.add_argument("-td","--maxneutralshift", type=float, required=False, default=config.compute_cn.maxneutralshift, help="Maximum BAF shift for neutral cluster used to automatically infer the diploid/tetraploid cluster (default: 0.1)")
    parser.add_argument("--merge", action='store_true', default=config.compute_cn.merge, required=False, help="Merge the clusters (default: false)")
    parser.add_argument("-mR","--mergeRDR", type=float, required=False, default=config.compute_cn.mergerdr, help="RDR tolerance used for finding the clonal copy numbers (default: 0.08)")
    parser.add_argument("-mB","--mergeBAF", type=float, required=False, default=config.compute_cn.mergebaf, help="BAF tolerance used for finding the clonal copy numbers (default: 0.04)")
    parser.add_argument("-l", "--limitinc", type=float, required=False, default=config.compute_cn.limitinc, help="Upper bound to the relative increase of objective function. When there are significant small CNAs, their effect on the objective function may be confounded by only larger events, use this value to limit the relative increase of OBJ so that fitting small CNAs is more considered (default: None)")
    parser.add_argument("-g", "--ghostprop", type=float, required=False, default=config.compute_cn.ghostprop, help="Increasing proportion used to compute the value of the first ghost point added in the solution selection (default: 0.3)")
    parser.add_argument("-tR","--toleranceRDR", type=float, required=False, default=config.compute_cn.tolerancerdr, help="RDR tolerance used for finding the clonal copy numbers (default: 0.08)")
    parser.add_argument("-tB","--toleranceBAF", type=float, required=False, default=config.compute_cn.tolerancebaf, help="BAF tolerance used for finding the clonal copy numbers (default: 0.04)")
    parser.add_argument("-p","--seeds", type=int, required=False, default=config.compute_cn.seeds, help="Number of seeds for coordinate-descent method (default: 400)")
    parser.add_argument("-j","--jobs", type=int, required=False, default=config.compute_cn.jobs, help="Number of parallel jobs (default: maximum available on the machine)")
    parser.add_argument("-r","--randomseed", type=int, required=False, default=config.compute_cn.randomseed, help="Random seed (default: None)")
    parser.add_argument("-s","--timelimit", type=int, required=False, default=config.compute_cn.timelimit, help="Time limit for each ILP run (default: None)")
    parser.add_argument("-m","--memlimit", type=int, required=False, default=config.compute_cn.memlimit, help="Memory limit for each ILP run (default: None)")
    parser.add_argument("-u","--minprop", type=float, required=False, default=config.compute_cn.minprop, help="Minimum clone proporion in each sample (default: 0.03)")
    parser.add_argument("--maxiterations", type=int, required=False, default=config.compute_cn.maxiterations, help="Maximum number of iterations composed of C-step/U-step for each seed (default: 10)")
    parser.add_argument("--mode", type=int, required=False, default=config.compute_cn.mode, help="Solving mode among: Coordinate Descent + exact ILP (0), exact ILP only (1), and Coordinate-descent only (2) (default: 2)")
    ...
    if args.minchrs < 0 or args.minchrs > 22:
        raise ValueError(error("The number of chromosomes for potential clonal clusters must be in {0, .., 22}!"))
    if args.maxneutralshift < 0.0 or args.maxneutralshift > 1.0:
        raise ValueError(error("The maximum BAF shift for neutral cluster must be in [0, 1]!"))
    if args.toleranceRDR < 0.0 or args.toleranceRDR > 1.0:
        raise ValueError(error("The RDR tolerance for finding clonal copy numbers must be in [0, 1]!"))
    if args.toleranceBAF < 0.0 or args.toleranceBAF > 1.0:
        raise ValueError(error("The BAF tolerance for finding clonal copy numbers must be in [0, 1]!"))
...
def readBBC(filename):
    ...
    for c in bbc:
        for seg in bbc[c]:
            for p in samples:
                assert p in bbc[c][seg], error("ERROR: bin ({}:{}-{}) not defined in sample {}".format(c, seg[0], seg[1], p))
...
def readSEG(filename):
    ...
    for i in seg:
        for p in samples:
            assert p in seg[i], error("ERROR: cluster {} not defined in sample {}".format(i, p))
...
def computeSizes(seg, bbc, samples):
    ...
    size = {idx : sum( float(b[1] - b[0]) for c in bbc for b in bbc[c] if bbc[c][b][sample]['cluster'] == idx) for idx in seg}
    for idx in seg:
        for p in samples:
            seg[idx][p]['avgbaf'] = sum(bbc[c][b][p]['baf'] * float(b[1] - b[0]) for c in bbc for b in bbc[c] if bbc[c][b][p]['cluster'] == idx) / size[idx]
    return size
...
def filtering(bbc, seg, size, ts, tc, mB, mR, samples, v):
    ...
    fbbc = {c : {seg : bbc[c][seg] for seg in bbc[c] if bbc[c][seg][sample]['cluster'] in selected} for c in bbc}
...
def findNeutralCluster(seg, size, td, samples, v):
    selected = set(idx for idx in seg if set(p for p in samples if abs(0.5 - seg[idx][p]['baf']) <= td) == samples)
    if len(selected) == 0:
    ...
    if v >= 2:
        sys.stderr.write(info("## Cluster selected as neutral (diploid/tetraploid) is {}\n".format(neutral)))
...
def findClonalClusters(fseg, neutral, size, tB, tR, samples, v):
        ...
        for opt in options:            
            ...
            if v >= 2:
                if curr_pattern not in found_pattern:
                    sys.stderr.write(info("## Found pattern of size {}: {}\n".format(curr_value, curr_pattern)))
            if curr_pattern not in found_pattern:
    ...
    if v >= 1:
        sys.stderr.write(info("## Chosen pattern of size {}: {}\n".format(best_value, best_pattern)))
