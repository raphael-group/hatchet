...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default=config.preprocess.size,
        help='Bin size, with or without "kb" or "Mb" (default: 250kb)',
    )
    parser.add_argument(
        "-c",
        "--minreads",
        type=int,
        required=False,
        default=config.preprocess.minreads,
        help="Minimum read counts for heterozygous germline SNPs (default: 8)",
    )
    parser.add_argument(
        "-C",
        "--maxreads",
        type=int,
        required=False,
        default=config.preprocess.maxreads,
        help="Maximum read counts for heterozygous germline SNPs (default: 1000)",
    )
    parser.add_argument(
        "-p",
        "--phred",
        type=int,
        required=False,
        default=config.preprocess.phred,
        help="Phred quality score (default: 11)",
    )
    parser.add_argument(
        "-x",
        "--rundir",
        required=False,
        default=config.preprocess.rundir,
        type=str,
    ...
    parser.add_argument(
        ...
        required=False,
        type=int,
        default=config.preprocess.seed,
        help="Random seed for replication (default: None)",
    )
    parser.add_argument(
        "-j",
        "--jobs",
...
def main():
    ...
    cmd = cmd.format(
        get_comp("count_reads.py"),
        args["normal"],
        " ".join(args["tumor"]),
        "normal " + " ".join(args["names"]),
        args["size"],
        args["ref"],
        args["J"],
        args["phred"],
