...
| Demo | Description |
|------|-------------|
| [demo-complete](examples/demo-complete/demo-complete.md) | A demo of the complete HATCHet pipeline starting from an example dataset of tumour and matched normal BAM files |
| [demo-wgs-sim](examples/demo-WGS-sim/demo-wgs-sim.md) | A demo on a typical WGS (whole-genome sequencing) multi-sample dataset with standard noise and variance of the data |
| [demo-wgs-cancer](examples/demo-WGS-cancer/demo-wgs-cancer.md) | A demo on a cancer WGS (whole-genome sequencing) multi-sample dataset with high noise and variance of the data |
| [demo-wes](examples/demo-WES/demo-wes.md) | A demo on a cancer WES (whole-exome sequencing) multi-sample dataset, which is typycally characterized by very high variance of RDR |
...
|------|-------------|--------|------|------------|
| GATK4-CNV | Custom pipeline for segmented files from GATK4 CNV pipeline | [custom-gatk4-cnv.sh](custom-gatk4-cnv.sh) | [demo-gatk4-cnv.sh](custom/GATK4-CNV/demo-gatk4-cnv.md) | This custom pipeline takes the input the segmented files which already contain the estimated RDR and BAF. As such, the first pre-processing steps of HATCHet (`count-reads`, `count-alleles`, and `combine-counts`) are not needed; for this reason, the following dependencies SAMtools and BCFtools and the following required data, human reference genome, matched-normal sample, and BAM files, are not needed in this case. |
...
|-------|------|----------|-------------|
| (1)   | [*genotype-snps*](doc_genotype_snps.md)   | deBAF    | This step calls heterozygous germline SNPs from the matched-normal sample. |
| (2)   | [*count-alleles*](doc_count_alleles.md)   | deBAF    | This step counts the number of reads covering both the alleles of each identified heterozygous SNP in every tumor sample. |
| (3)   | [*count-reads*](doc_count_reads.md)       | N/A   | This step splits the human reference genome into very small regions (representing candidate bins) according to germline SNP positions, and counts the number of total reads aligned to each region in each of the tumor samples and in the matched normal sample. |
| (4)   | [*phase-snps*](doc_phase_snps.md)       | N/A   | This **optional** step uses reference-based phasing to group together germline SNPs that are likely on the same haplotype. This improves the quality of BAF estimates for balanced regions of the genome. |
| (5)   | [*combine-counts*](doc_combine_counts.md) | N/A   | This step constructs genomic bins with variable sizes, and combines the read counts and the allele counts for the identified germline SNPs to compute the read-depth ratio (RDR) and B-allele frequency (BAF) of every genomic bin. |
| (6)   | [*cluster-bins*](doc_cluster_bins.md)  | N/A    | This step globally clusters genomic bins along the entire genome and jointly across tumor samples while incorporating local information. |
| (7)   | [*plot-bins*](doc_plot_bins.md)           | BBot     | This **optional**  step produces informative plots concerning the computed RDRs, BAFs, and clusters. The information produced by this step are important to validate the compute clusters of genomic regions. When this step is enabled in the `.ini` file, [plot-bins-1d2d](doc_plot_bins_1d2d.md) is run as well to generate additional plots. |
| (8)   | [*compute-cn*](doc_compute_cn.md)         | hatchet  | This step computes allele-specific fractional copy numbers, solves a constrained distance-based simultaneous factorization to compute allele and clone-specific copy numbers and clone proportions, and deploys a model-selection criterion select the number of clone by explicitly considering the trade-off between subclonal copy-number aberrations and whole-genome duplication. |
| (9)   | [*plot-cn*](doc_plot_cn.md)               | BBeval   | This **optional** step analyzes the inferred copy-number states and clone proportions and produces informative plots jointly considering all samples from the same patient. In addition, this step can also combine results obtained for different patients and perform integrative analysis. When this step is enabled in the `.ini` file, [plot-cn-1d2d](doc_plot_cn_1d2d.md) is run as well to generate additional plots. |
| (10)   | [*check*](doc_check.md)     | check-solver           | This **optional** step is a quick way to verify if your solver and other 3rd party dependencies are working correctly. It checks for the presence of several dependencies and tests `compute_cn` on a set of small data files pre-packaged.  |
...
| [Analyze global clustering](recommendation_clustering.md) | Interpret global clustering, quality and error control, and parameter tuning |
